...
if(/[?&]showcase=1/.test(location.search)){
  (function(){
    ...
    const PROBE = [
      ...
      '#cam-panel',        // 章4
      '#gizmo',            // 章2（測定パネル）
      '#layer-transform', '#lp-footer-bar', '#lp-divider',   // 章6（レイヤー展開）
      '#joy', '#joy-vert',
    ];
    ...
    function obstacleRects(){
      ...
      for(const sel of OBSTACLES){
        ...
        for(const el of els){
          ...
          // 祖先ごと隠れている場合も除外（読込前の #hud{opacity:0} 配下など）
    ...
    async function run(){
      ...
      // チップの定位置をここで 1 回だけ決める（以後は動かさない）。
      // UI が出そろってから測る必要があるので、シーン読込後のこの位置。
      try{ layoutChip(); }catch(e){ console.warn('[showcase] layout failed', e); }
