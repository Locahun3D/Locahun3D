...
(() => {
  globalThis.LocahunNavigationRegionLoader={create({baseUrl,fetchFn=fetch,timeoutMs=5000,extension='lnv'}){
    if(extension!=='lnv'&&extension!=='lcp')throw Error('Invalid navigation extension');
    ...
    return async(entry,signal)=>{
      if(!entry||typeof entry.key!=='string'||!/^[a-f0-9]{64}$/.test(entry.key)||!Number.isInteger(entry.bytes)||entry.bytes<1||entry.bytes>2000000)throw Error('Invalid navigation entry');
