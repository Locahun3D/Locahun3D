// Bounded preflight using the same controller as actual click travel.
globalThis.LocahunRouteClearance=function(points,core,Navigation){
  ...
  for(const route of [points,[...points].reverse()]){
   ...
   const nav=Navigation.create({position:()=>position,setPosition:p=>position={...p},ready:()=>true,blocked:()=>false,epoch:()=>1,coverage:()=>true,
    clear:p=>{
     ...
     return valid(hit?.point)&&valid(hit?.normal)&&hit.normal.y>=.7&&Math.abs(hit.point.y-feet)<=.3&&core.isCapsuleClear({x:p.x,y:feet+.3,z:p.z},1.7,.15);
