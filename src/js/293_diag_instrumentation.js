...
// Exercise the real local-file drag-drop path (loadSplatFile is module-
// scoped, not otherwise reachable from devtools) — lets a test pass a real
// File object (e.g. wrapping fetched .rad bytes) through the exact same
// code a drag-drop would run, including the new blob:-URL RAD streaming.
window.__testLoadSplatFile = function(file){ return loadSplatFile(file); };
...
// `pixelScaleLimit *= this.lodRenderScale`). Lower = finer/more detail.
window.__setLodRenderScale = function(n){
  window.__lodRenderScalePin = (typeof n === 'number' && n > 0) ? n : null;
  try{ if(typeof sparkRenderer !== 'undefined' && sparkRenderer && window.__lodRenderScalePin){
    sparkRenderer.lodRenderScale = window.__lodRenderScalePin;
    window.__lodRenderScaleReadback = sparkRenderer.lodRenderScale;
  }}catch(e){ window.__lodRenderScaleErr = String(e && e.message || e); }
};
setInterval(()=>{
  if(window.__lodRenderScalePin == null) return;
  try{ if(typeof sparkRenderer !== 'undefined' && sparkRenderer){
    sparkRenderer.lodRenderScale = window.__lodRenderScalePin;
    window.__lodRenderScaleReadback = sparkRenderer.lodRenderScale;
  }}catch(_){}
}, 500);
